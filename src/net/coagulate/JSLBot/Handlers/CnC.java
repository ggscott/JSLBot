...
public class CnC extends Handler {
	...
	boolean hasCoffed;
	@Nullable private   Date       evacby;
	private             int        pauseserial;
	@Nullable private   Date       homesickness;
	
	public CnC(@Nonnull final JSLBot bot,@Nonnull final Configuration config) {
		super(bot,config);
		...
		try {
			auth=(Authorisation)Class.forName(authoriser)
			                         ...
			                         .newInstance(bot,config.subspace("authorisation"));
			...
			if ("RegionRestartMinutes".equals(infotype)||"RegionRestartSeconds".equals(infotype)) {
				...
				if (seconds<=60) {
					...
					if (evacby==null||evacby.before(new Date())) { // if not evacuating or it was in the past
						evacby=new Date(when.getTime()+
						                30000); // set to 30 seconds post evacuation so we dont do this more than once
						@Nonnull final Map<String,String> params=new HashMap<>();
						params.put("when",String.valueOf((int)(when.getTime()/1000)));
						@Nullable final CommandEvent evacuate=
								new CommandEvent(bot,event.region(),"evacuate",params,null);
						evacuate.submit();
					}
				}
				...
				log.log(level,
				        "Simulator "+event.region().circuit()+" will shut down in "+mins+"m"+secs+"s at "+
				        datetime.format(when));
			}
			if (!handled) {
				log.warning("Unhandled warning from "+event.region().circuit()+" included info:"+info.vmessage+" / "+
				            info.vextraparams);
			}
		}
	}
	...
	private static Date parseRegionRestart(@Nonnull final String m) {
		if (m.split("\n").length<2) {
			throw new IllegalArgumentException("Expected at least 2 lines of input");
		}
		final String line=m.split("\n")[1];
		@Nonnull final LLSDMap msg=(LLSDMap)new LLSD(line).getFirst();
		if (msg==null) {
			throw new IllegalArgumentException("Failed to parse LLSDMap from region restart message");
		}
		...
		if (msg.containsKey("SECONDS")) {
			shutdown=shutdown+((Integer.parseInt(msg.get("SECONDS").toString()))*1000L);
		}
	...
	public void runCommands(final String from,
		...
		try {
			...
			if (response==null) {
				...
				log.warning("Caller "+from+" failed auth check : "+response);
			}
		} catch (@Nonnull final Exception e) {
		...
		for (int i=index;i<parts.length;i++) {
			if (keyword!=null&&keyword.isEmpty()) {
				...
				if ((!parameter.toString().startsWith("\""))||
				    (parameter.toString().startsWith("\"")&&parameter.toString().endsWith("\""))) {
					if (parameter.toString().startsWith("\"")) {
	...
	public void establishAgentCommunicationXMLDelayed(@Nonnull final XMLEvent event) {
		...
		for (@Nonnull final Circuit c: bot.getCircuits()) {
			if (c.getSimIPAndPort().equalsIgnoreCase(simipandport)) {
				if (Debug.EVENTQUEUE) {
					log.fine("Matched ip and port to circuit for region "+c.getRegionName());
				}
	...
	public String helpCommand(final CommandEvent commandevent,
	                          @Nullable @Param(name="command", description="Optional command to get more info about")
	                          String command) {
	...
	@Nonnull
	private String chat(final int messagetype,@Nonnull final String message) {
	...
	*/
	...
	public String shoutCommand(final CommandEvent event,
	                           @Param(name="message", description="Message to shout") @Nonnull final String message) {
	...
	public String whisperCommand(final CommandEvent event,
	                             @Param(name="message", description="Message to whisper") @Nonnull
	...
	public void maintenance() {
		if (bot.homeSickFor()==null) {
			return;
		} // we dont get homesick
		if (bot.getRegionName()==null||bot.getRegionName().isEmpty()) {
			return;
		} // we're nowhere (early login phase)
		if (homesickness!=null&&homesickness.after(new Date())) {
			return;
		} // if timer set, wait for it to pass
	...
	public String homesickCommand(final CommandEvent event,
	                              @Nullable
	                              @Param(name="region",
	                                     description="Name of region to long for, blank to get current, or NONE to clear")
	                              final String region) {
		if (region==null||region.isEmpty()) {
			@Nullable final String home=bot.homeSickFor();
			if (home==null||home.isEmpty()) {
				return "Bot has no longing for any home";
			}
	...
	public String setSecretKeyCommand(final CommandEvent command,
	                                  @Nonnull
	                                  @Param(name="secretkey", description="Secret key to use (make it secure!)")
	                                  final String secretKey) {
		bot.getConfig().put("secretkey",secretKey);
		if ("password".equalsIgnoreCase(secretKey)) {
			return "No! Bad Human!";
		}
		return bot.getConfig().persistent()?"Secret key changed and saved":
		       "Secret key changed ; note your configuration store "+bot.getConfig().getClass().getSimpleName()+
		       " is not persistent and changes will be lost on restart.";
	}
	...
	public void avatarAppearanceUDPDelayed(@Nonnull final UDPEvent event) {
		...
		try {
			...
			if (matcher.matches()) {
				...
				log.log(WARNING,"Cof retrier failed to match error message:"+error);
			}
