...
public class Inventory extends Handler implements Runnable {
	...
	private final Map<LLUUID,Set<LLUUID>>   inventorytree=new HashMap<>();
			// parent id -> set of descendants - items OR folders
	...
	public void run() {
		while (!downloadqueue.isEmpty()) {
			...
			final int percent=
					(int)(Math.round((100.0*inventorytree.size())/(inventorytree.size()+downloadqueue.size())));
			log.fine("Inventory download: ["+percent+"%] "+inventorytree.size()+" branches complete, "+
			         downloadqueue.size()+" to go ("+inventory.size()+" elements)");
		}
	...
	private void fetchInventory(@Nonnull final Set<LLUUID> uuids) throws IOException {
		...
		if (debugqueries) {
			System.out.println("Querying "+uuids.size()+" uuids");
		}
		...
		if (debugqueries) {
			System.out.println("Outer array:"+folders.get().size());
		}
		for (final Atomic a: folders.get()) {
			...
			if (debugqueries) {
				System.out.println("Inner items:"+inneritems.get().size());
			}
			...
			if (debugqueries) {
				System.out.println("Inner categories:"+innercategories.get().size());
			}
		...
		if (query.isEmpty()) {
			if (debugqueries) {
				System.out.println("No new queries to generate");
			}
		} else {
			if (debugqueries) {
				System.out.println("Generated new queries:"+query.size());
			}
	...
	public String inventoryFindCommand(final CommandEvent event,
	                                   @Nonnull @JSLBot.Param(description="Substring of item name to find", name="name")
	                                   final String name) {
		synchronized(inventorytree) {
	...
	@Nonnull
	@CmdHelp(description="Send an inventory item to a recipient")
	public String inventorySendCommand(final CommandEvent event,
	                                   @Nonnull @JSLBot.Param(description="UUID of item to send", name="item")
	                                   final String item,
	                                   @Nonnull
	                                   @JSLBot.Param(description="UUID of avatar to send item to", name="target")
	                                   final String target) {
		if (!inventorycomplete) {
			return "2 - Inventory download is not yet complete";
		}
	...
	private InventoryItem inventoryFind(@Nonnull final LLUUID uuid) {
		if (!inventorycomplete) {
			throw new IllegalStateException("Unable to search inventory, not yet completed initial download");
		}
		synchronized(inventorytree) {
			for (final Map.Entry<LLUUID,InventoryAtom> entry: inventory.entrySet()) {
	...
	public void sendInventory(@Nonnull final LLUUID itemuuid,@Nonnull final LLUUID target) {
		...
		if (item==null) {
			throw new NullPointerException("Unable to resolve UUID "+itemuuid.toUUIDString());
		}
	...
	public String inventoryDetailCommand(final CommandEvent event,
	                                     @Nonnull @JSLBot.Param(description="UUID of item to report on", name="uuid")
	                                     final String uuid) {
		if (!inventorycomplete) {
			return "2 - Inventory download is not yet complete";
		}
