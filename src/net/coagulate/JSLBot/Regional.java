...
public class Regional {
	...
	@Nullable private LLVector3               sundirection;
	...
	public void killObject(final int value) {
	...
	public ObjectData getObject(final int id) {
		synchronized(objects) {
			if (!objects.containsKey(id)) {
				objects.put(id,new ObjectData(circuit.bot(),id));
			}
	...
	public void setCoarseAgentLocations(final Map<LLUUID,LLVector3> locmap) {
	...
	public ParcelData getParcel(final int get) {
		if (!localparcelids.containsKey(get)) {
			localparcelids.put(get,new ParcelData(get,this));
		}
		return localparcelids.get(get);
	}
	
	public Integer getResponse(final int requestid) {
		return requestresponses.get(requestid);
	}
	
	public void requestResponse(final int sequenceid,final int get) {
		requestresponses.put(sequenceid,get);
	}
	
	public void setParcelMap(final int sequence,final byte b) {
	...
	public long getDayUSec() {
		return dayusec;
	}
	
	public void setDayUSec(final long dayusec) {
		this.dayusec=dayusec;
	}
	
	@Nullable
	public LLVector3 getSunDirection() {
		return sundirection;
	}
	
	public void setSunDirection(final LLVector3 sundirection) {
		this.sundirection=sundirection;
	}
	
	public float getSunPhase() {
		return sunphase;
	}
	
	public void setSunPhase(final float sunphase) {
	...
	public static class ParcelData {
		...
		public final          int      id;
		@Nonnull public final Regional region;
		public                boolean  requested;
		public                boolean  populated;
		public                int      ownerprims       =-1;
		public                int      groupprims       =-1;
		public                int      otherprims       =-1;
		@Nullable public      String   musicurl;
		@Nullable public      LLUUID   group;
		...
		public String toString() {
			...
			if (populated) {
				ret+=" '"+name+"' ("+description+") prims:"+ownerprims+"/"+groupprims+"/"+otherprims+"="+primsused+
				     " of "+maxprims+" (simmax:"+simwidetotalprims+") Privacy:"+(!seeavs)+" claimed "+
				     new Date(((long)(claimdate))*1000);
			}
			return ret;
		}
