...
public class JSLBot extends Thread {
	...
	@Nonnull private final Brain                brain;
	...
	@Nullable private String  homesickfor;
	@Nullable private LLUUID  inventoryroot;
	// ********** BOT STARTUP **********
	private           boolean connected;
	@Nullable private Date    lastagentupdate;
	 ...
	 */
	@Nullable private Circuit primary;
	private           float   x;
	private           float   y;
	private           float   z;
	private           float   lx;
	...
	private void loadConf(final Configuration conf) {
		...
		@Nullable String uri=config.get("loginuri");
	...
	public void dumpAccounting() {
		...
		synchronized(messagebytesin) {
			for (@Nonnull final Map.Entry<Integer,Integer> entry: messagebytesin.entrySet()) {
				System.out.println("Message ID : "+entry.getKey()+" received "+entry.getValue());
			}
		}
		synchronized(messagebytesout) {
			for (@Nonnull final Map.Entry<Integer,Integer> entry: messagebytesout.entrySet()) {
	...
	public void shutdown(final String reason) {
		...
		@Nonnull final Set<Circuit> closeme=new HashSet<>(getCircuits());
		for (@Nonnull final Circuit c: closeme) {
			try {
				c.close();
	...
	@Nullable
	public LLUUID getInventoryRoot() {
	...
	public JSLInterface api() {
		if (jslinterface==null) {
			throw new NullPointerException("JSLInterface not yet initialised");
		}
		return jslinterface;
	}
	...
	public void waitConnection(final long milliseconds) {
		if (quit) {
			throw new IllegalStateException("Quitting, can not wait on connection");
		}
		if (connected) {
			return;
		}
		try {
			synchronized(connectsignal) {
				connectsignal.wait(milliseconds);
			}
	...
	public void run() {
		...
		if (brain.isEmpty()) {
			log.warning("Bot has no brain and will be a virtual zombie.");
		}
		reconnect=true;
	...
	public void completeAgentMovement() {
		@Nonnull final CompleteAgentMovement p=new CompleteAgentMovement();
	...
	public void agentUpdate(final boolean force) {
		...
		@Nonnull final AgentUpdate p=new AgentUpdate();
		p.bagentdata.vagentid=getUUID();
		p.bagentdata.vsessionid=getSession();
		@Nonnull final LLVector3 camera=getPos();
		camera.z+=5;
		if (blind) {
	...
	public LLUUID getUUID() {
		return uuid;
	}
	
	public LLUUID getSession() {
		return sessionid;
	}
	
	@Nonnull
	public LLVector3 getPos() {
	...
	public void send(@Nonnull final Packet p) {
		if (primary==null) {
			throw new IllegalStateException("Primary circuit is not defined or connected");
		}
		try {
			primary.send(p);
	...
	public void im(final LLUUID uuid,@Nonnull final String message) {
		@Nonnull final ImprovedInstantMessage reply=new ImprovedInstantMessage(this);
	...
	Circuit circuit() {
		if (primary==null) {
			throw new IllegalStateException("Primary circuit is not yet set up");
		}
		return primary;
	}
	 ...
	 */
	public void setPrimaryCircuit(final Circuit c) {
		primary=c;
	}
		...
		synchronized(circuits) {
			if (circuits.containsKey(handle)) {
				if (circuits.get(handle).isAlive()) {
					...
					if (Debug.CIRCUIT) {
						log.fine("Duplicate circuit to "+handle+" ignored");
					}
					return circuits.get(handle);
				}
			}
			if (Debug.CIRCUIT) {
				log.fine("New circuit to "+handle);
			}
			@Nonnull final Circuit newcircuit=new Circuit(this,numericip,port,handle,capsurl);
			newcircuit.connect();
	...
	public Set<Regional> getRegionals() {
		@Nonnull final Set<Regional> regionalset=new HashSet<>();
		synchronized(circuits) {
			for (@Nonnull final Circuit circuit: circuits.values()) {
	...
	@Nonnull
	public LLVector3 getLookAt() {
		return new LLVector3(lx,ly,lz);
	}
	...
	public void setMaxFOV() {
		setFOV((float)(Math.PI));
	}
	
	public void setFOV(final float angle) {
		@Nonnull final AgentFOV fov=new AgentFOV();
	...
	void accountMessageIn(final int id,final int length) {
		synchronized(messagebytesin) {
			int sum=0;
			if (messagebytesin.containsKey(id)) {
				sum=messagebytesin.get(id);
			}
			sum+=length;
	...
	void accountMessageOut(final int id,final int length) {
		synchronized(messagebytesout) {
			int sum=0;
			if (messagebytesout.containsKey(id)) {
				sum=messagebytesout.get(id);
			}
			sum+=length;
	 ...
	 */
	void deregisterCircuit(final Long regionhandle,final Circuit circuit) {
		synchronized(circuits) {
	...
	private void performLogin(final String firstname,
		...
		for (int retries=0;retries<Constants.MAX_RETRIES;retries++) {
			try {
				...
				if (delay>Constants.MAX_RETRY_INTERVAL) {
					delay=Constants.MAX_RETRY_INTERVAL;
				}
				if (e instanceof NullPointerException) {
					log.log(SEVERE,"Unexpected null pointer exception during login",e);
				} else {
					log.info("Login attempt "+(retries+1)+"/"+Constants.MAX_RETRIES+" failed: "+
	...
	private void login(final String firstname,
		...
		@Nonnull final Map<Object,Object> result=
				BotUtils.loginXMLRPC(this,firstname,lastname,password,loginlocation,loginuri);
	...
	private void mainLoop() throws Exception {
		...
		if (!config.get("homeseat","").isBlank()) {
			...
			@Nonnull final CommandEvent sit=
					new CommandEvent(this,null,"siton",args,new LLUUID(config.get("CnC.authorisation.owneruuid")));
			brain.execute(sit);
		}
		while (!quit) {
			//agentUpdate();
	...
	private static class ShutdownHook extends Thread {
		...
		@Override
		public void run() {
			bot.shutdown("JVM called shutdown hook (program terminated?)");
		}
	}
}
